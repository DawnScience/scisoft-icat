...
public class CNFContentProvider implements ITreeContentProvider {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(CNFContentProvider.class);

	@Override
	public Object[] getChildren(Object parentElement) {
		logger.info("object triggered is of type ---> "
		...
		if (parentElement instanceof Root) {
			...
			try {
				logger.debug("populating dataset id: "
						+ ((DDataset) parentElement).getId());
				String sessionId = ICATSessionDetails.icatClient.getSessionId();

				long startTime = System.currentTimeMillis();
				Dataset dataset = ICATClient.getIcat()
						.getDatasetIncludes(sessionId,
								((DDataset) parentElement).getId(),
								DatasetInclude.DATASET_AND_DATAFILES_ONLY);

				// OpenDatasetJob openDataset = new
				// OpenDatasetJob("populating dataset: " +
				// ((DDataset)parentElement).getName(), sessionId,
				// ((DDataset)parentElement));
				// openDataset.setUser(false);// hides the dialog for 'run in
				...
				ddatafiles = new Datafile[dataset.getDatafileCollection()
						.size()];
	...
	private void fillDInvestigations(Object parentElement) {
		...
		try {
			...
			filledInv = ICATClient.getIcat()
					.getInvestigationIncludes(sessionId, dinvId,
			...
			for (int i = 0; i < datasetColSize; i++) {
				Dataset dataset = filledInv.getDatasetCollection()
						.get(i);

				DDataset ddataset = new DDataset(dataset.getId(),
						dataset.getName(), dataset.getDatasetStatus(),
						dataset.getDatasetType(), dataset.getDescription(),
						dataset.getInvestigationId(), dataset.getLocation(),
	...
	public boolean hasChildren(Object element) {
		return (element instanceof Root || element instanceof DIcat
				|| element instanceof DInvestigation || element instanceof DDataset);
	}
	...
	private void fillDIcats(Object parentElement) {
		...
		if (ICATSessionDetails.icatClient != null) {
			...
			DIcat dicat = new DIcat(ICATSessionDetails.icatClient.getFedId(),
					ICATSessionDetails.icatClient.getIcatHost());
			this.dIcats[0] = dicat;
			this.dIcats[0].setRoot(parentElement);

			// call icat webserivce with current user and get list of
			// investigations
			List InvestigationsList = null;
			List<Investigation> datasetInvList = null;

			try {
				InvestigationsList = ICATSessionDetails.icatClient
						.getLightInvestigations();
			...
			for (int i = 0; i < children.length; i++) {
				...
				DInvestigation dinv = new DInvestigation(inv.getId(),
						inv.getVisitId());
